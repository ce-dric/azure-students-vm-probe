...
**언어:** [English](README.md) | [日本語](README.ja.md) | [中文](README.zh.md)

---

Azure for Students 구독에서 VM을 배포 가능한 리전과 사이즈 조합을 자동으로 탐색하고, 첫 번째 유효한 후보에 VM을 생성하는 스크립트입니다.

## 배경

Azure for Students 구독은 리전별 쿼터 제한이 있어 실제로 배포해보기 전까지 어떤 조합이 가능한지 알기 어렵습니다. 이 스크립트는 ARM 템플릿 `validate`를 활용해 과금 없이 배포 가능한 후보를 탐색하고, 선택적으로 VM을 생성합니다.
...
# 탐색 후 첫 번째 유효한 조합에 VM 생성
bash azure-students-vm-probe.sh
```

## 동작 방식

1. 설정된 리전 목록을 순회하며 리전별 테스트 리소스 그룹 생성
2. 각 VM 사이즈에 대해 ARM 템플릿 validate 수행 (배포 없음, 과금 없음)
3. 첫 번째로 유효한 리전 + 사이즈 조합 발견 시 탐색 종료
4. `DRY_RUN_ONLY`가 설정되지 않은 경우 해당 조합으로 VM 1개 배포
5. 전체 결과를 TSV 파일로 저장
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `REGIONS` | eastus, westus, japaneast, koreacentral 등 12개 | 탐색할 Azure 리전 목록 (공백 구분) |
| `SIZES` | Standard_B1s, B1ls, B1ms, B2ats_v2, A1_v2 | 시도할 VM 사이즈 목록 (공백 구분) |
| `ADMIN_USER` | `azureuser` | VM 관리자 계정명 |
| `SSH_PUBLIC_KEY_FILE` | `~/.ssh/id_ed25519.pub` | SSH 공개키 파일 경로 |
| `NAME_PREFIX` | `azvm` | 생성되는 리소스 이름 접두사 |
...
- 탐색 과정에서 테스트 리소스 그룹이 생성/삭제됩니다. `KEEP_FAILED_RESOURCE_GROUPS=1`로 유지할 수 있습니다.
- VM 생성 후에는 요금이 발생합니다. 사용 후 리소스 그룹을 삭제하세요:
  ```bash
...
이 레포에는 `SKILL.md`가 포함되어 있어 [k-skill](https://github.com/NomaDamas/k-skill)로 등록하면 Claude Code가 직접 탐색을 실행해줍니다.
...
**그 다음 Claude에게 말로 요청하면 됩니다:**

> "Azure Student 계정에서 배포 가능한 리전 찾아줘"
> "Azure for Students로 VM 하나 만들어줘"

Claude가 탐색을 실행하고 결과 테이블을 보고한 뒤, 필요하면 VM을 생성해줍니다.
